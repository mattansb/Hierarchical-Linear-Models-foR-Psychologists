...
# getting the DATA:
temp <- tempfile()
download.file("https://www.pilesofvariance.com/Chapter3a/SPSS/SPSS_Chapter3a.zip", temp)
...
mod_max.mixed <- lmer(outcome ~ Time * group + (1 | PersonID),
...
mode_max.fixed <- lm(outcome ~ Time * group,
                     data = dataset_long)



# We can directly compare the model with sjPlot::tab_model():
sjPlot::tab_model(mode_max.fixed, mod_max.mixed, 
                  dv.labels = c("No Random Effects", "Mixed Effects Model"),
                    
                  show.se = TRUE, show.df = TRUE,
...
emtrends(mod_max.mixed, # the model
