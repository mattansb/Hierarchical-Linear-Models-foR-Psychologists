...
mod_empty <- lm(rt ~ 1, data = dataset)
summary(mod_empty)
...
mod_rndm.sssn <- lmer(
  rt ~ session + (1 | PersonID) + (0 + session | PersonID),
  data = dataset
)

# Or use || do "remove" random covariance
mod_rndm.sssn <- lmer(rt ~ session + (session || PersonID), data = dataset)
...
mod_full <- lmer(rt ~ session + (session | PersonID), data = dataset)
