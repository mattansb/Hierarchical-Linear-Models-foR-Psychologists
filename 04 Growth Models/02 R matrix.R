...
mod_sssn.lme <- lme(rt ~ session,
                    random = ~ session | PersonID,
                    method = "REML",
                    data = dataset)

# Or:
mod_sssn.gTMB <- glmmTMB(rt ~ session + (session | PersonID),
                         REML = TRUE,
                         data = dataset)

# Or:
mod_sssn.brm <- brm(rt ~ session + (session | PersonID),
                    data = dataset)
...
mod_sssn.ar1.lme <- lme(rt ~ session, 
                        random = ~ session | PersonID,
...
mod_sssn.ar1.gTMB <- glmmTMB(rt ~ session + (session | PersonID) +
...
mod_sssn.ar1.brm <- brm(rt ~ session + (session | PersonID) +
                          ar(session, gr = PersonID),
                        data = dataset)
# Again using default priors - probably a bad idea...
loo::loo_compare(loo::loo(mod_sssn.ar1.brm), 
                 loo::loo(mod_sssn.brm)) |> 
