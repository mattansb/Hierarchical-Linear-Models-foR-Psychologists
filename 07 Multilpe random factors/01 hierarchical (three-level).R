...
mod_rndm.intr <- lmer(
  math ~ 1 + (1 | childid:schoolid) + (1 | schoolid),
  data = egsingle
)
...
p_child <- ggplot(egsingle, aes(grade, math)) +
  stat_smooth(
    aes(group = interaction(schoolid, childid)),
    method = "lm",
    se = FALSE,
    alpha = 0.2
  ) +
  stat_smooth(method = "lm", se = FALSE, color = "black") +
  labs(title = "Trend per Child (nested in School)", x = "Grade", y = "Math") +
  lims(y = c(-5, 5)) +
  theme_bw()
...
p_school <- ggplot(egsingle, aes(grade, math)) +
  ...
  stat_smooth(method = "lm", se = FALSE, color = "black") +
  labs(title = "Trend per School", x = "Grade", y = "Math") +
  lims(y = c(-5, 5)) +
  theme_bw()
...
mod_grade <- lmer(
  math ~ grade + (1 | childid:schoolid) + (1 | schoolid),
  data = egsingle
)
...
mod_rndm.grade <- lmer(
  math ~ grade + (grade | childid:schoolid) + (1 | schoolid),
  data = egsingle
)
...
ggplot(egsingle, aes(grade, math)) +
  stat_smooth(
    aes(group = schoolid, color = lowinc),
    method = "lm",
    se = FALSE,
    alpha = 0.2
  ) +
