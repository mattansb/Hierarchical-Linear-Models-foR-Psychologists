...
mod_maximal <- lmer(value ~ (treatment + hour_c) * phase + gender + age_c + 
                      (phase + hour_c | id),
                    data = obk.long)
...
as.data.frame(em.int) |> 
  ggplot(aes(phase, emmean, color = treatment, shape = treatment, group = treatment)) + 
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL),
                width = 0.1, color = "black",
                position = pd) + 
  geom_point(size = 3, position = pd) + 
  geom_line(position = pd) + 
  scale_color_manual("Group", labels = c("Control", "A", "B"), 
